...
def apply_rotary(
    ...
    # Dynamic BLOCK_K based on rotary_dim
    BLOCK_K = max(8, triton.next_power_of_2(rotary_dim))
    BLOCK_K = min(BLOCK_K, 256)
    # Dynamic BLOCK_M based on seqlen
    BLOCK_M = max(4, triton.next_power_of_2(seqlen))
    BLOCK_M = min(BLOCK_M, 8)
